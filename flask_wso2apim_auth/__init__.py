...
# HTTP header providing the JSON Web Token
JWT_ASSERTION_HEADER = 'X-JWT-Assertion'

JWT_DEFAULT_PUBLIC_KEY_FILE = 'default-public-key.pem'
...
def decode_token():
    if JWT_ASSERTION_HEADER in request.headers:
        ...
        return jwt.decode(token,
                          current_app.config['WSO2AUTH_PUBLIC_KEY'],
                          algorithms=current_app.config['WSO2AUTH_ALGORITHM'],
                          verify=current_app.config['WSO2AUTH_VERIFY_TOKEN'])
    else:
...
class GTWSO2Auth(object):
    ...
    def init_app(self, app):
        app.config.setdefault('WSO2AUTH_VERIFY_TOKEN', True)
        app.config.setdefault('WSO2AUTH_ALGORITHM', 'RS256')
        ...
        # read default public key used to sign JWT (from api.rnoc.gatech.edu)
        public_key = pkg_resources.resource_string(__name__, JWT_DEFAULT_PUBLIC_KEY_FILE)
        app.config.setdefault('WSO2AUTH_PUBLIC_KEY', public_key)
